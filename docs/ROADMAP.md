...
| **当前阶段：v0.1**（含 1 条 v0.1 阶段做完整 TUI 之外的关键工作：**冻结最小 CLI/TUI 输出契约**——只冻结契约不实现完整 Textual TUI）。
**第一目标**：让最简版本 simple CLI 跑通端到端任务 + CLI 输出可读到能看清 Agent 在做什么，**不要被 slash command / 复杂 topic switch / LLM 意图分类 / 完整 Textual TUI / Skill 化 / 安全围栏 / observer 等高级能力带偏**。
...
| 核心测试可跑 | ✅ 269 passed / 3 xfailed | 一键 `pytest` 通过，xfail 全部明确归类 |
...
- 5 条毕业标准全部 ✅ 且下面 3 条 blocking 全关闭 → **立即冻结 v0.1 范围**，新功能必须先归类到 v0.2/v0.3/v1.0 backlog
