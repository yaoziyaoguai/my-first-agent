...
**最后更新**: 2026-05-30 (003 hardening: 4 PASS / 7 SKIP — H1/H2 architecture-limited (TOOL_REGISTRY 无 diverse disallowed tools); H3 safety assertions reconfirmed 3/3 PASS; 新增 caveat (5): blocking 证据限于单一 SKILL_SELECT)
...
| REAL-EVIDENCE-003 | Skill allowed_tools | **partial-credible / code-path credible with blocking demonstrated** | Real provider disallowed-tool blocking 5/5 PASS (原始) + 003 hardening 4 PASS / 7 SKIP (`scripts/real_evidence_003_hardening.py`)。H1/H2: SKIP — TOOL_REGISTRY 无除 SKILL_SELECT 外的 disallowed 业务工具；H3: safety assertions 3/3 PASS reconfirmed。**Caveats**: (1) adversarial prompt 极度 prompt-steered: 直接要求调用 SKILL_SELECT 并设置 blog-writing; (2) 被阻断工具是 SKILL_SELECT 自身，非更自然的 disallowed 业务工具; (3) policy_path/rejection_reason 在 handler payload 中不在 RuntimeActionEvent.evidence，当前用 decision=rejected + skill_allowed_tools present 作为语义等价代理; (4) 单 skill/单工具/单 adversarial prompt 场景; (5) TOOL_REGISTRY 无 diverse disallowed 业务工具——H1 (diverse tools) 和 H2 (adversarial styles) 无法在现有工具集验证（架构限制，非逻辑缺陷）。 |
