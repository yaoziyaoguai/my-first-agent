...
**最后更新**: 2026-05-31 (003 hardening v3: 10 PASS / 0 FAIL / 7 CONCERN / 0 SKIP — active_skill_id evidence 修复完成; R36 evidence completeness PASS; 003 升级为 credible-with-caveats)
**状态**: 阶段性收口。Score 4.5/5 conservative baseline with caveats。Evidence collected: 8/8。Credible: 7/8 (001/002/004/005/006/007/008)。Credible-with-caveats: 1/8 (003, see caveats below)。002: **credible** — Plan 3 pipeline + manifest-based language matching — 12/12 PASS real provider dogfood (AnthropicCompatibleProvider)。003: **credible-with-caveats** — lifecycle→mediator→gate allowed_tools enforcement 全链闭合, multiple disallowed business tools blocked, active_skill_id evidence 完整性已验证 (R36 PASS), 27 focused tests PASS。**Remaining caveats**: (1) prompt-steered adversarial prompts; (2) request_user_input 模型主动规避 (defense-in-depth); (3) 单 skill 场景; (4) H2 对抗风格全部 CONCERN (强模型自主规避限制 gate-only path 测试覆盖率)。006: **credible — 完整 evidence chain 闭合 (12/12 PASS)**。007: **credible — runtime-mediated execution chain 已验证** (10/10 PASS)。008: **credible — evidence chain fully closed, 最后一个 caveat 已关闭 (2026-05-31 v3)**。
...
| 当前 independent combined review score | 4.5/5 — conservative baseline with caveats。002 credible — Plan 3 pipeline (12/12 PASS, ab013ed) with scope caveats (prompt-steered, single-skill)。003 credible-with-caveats — lifecycle→mediator→gate enforcement 全链闭合, multiple disallowed business tools blocked (read_file/read_file_lines/SKILL_SELECT), active_skill_id evidence 完整性已验证 (R36 PASS), 10 PASS / 0 FAIL / 7 CONCERN (real AnthropicCompatibleProvider, 2026-05-31), 27 focused tests。006 credible — child_tools schema fix 闭合 MODEL_BEHAVIOR_CONCERN (12/12 PASS)。007 credible (10/10 PASS)。008 credible — evidence chain fully closed (v3, 2026-05-31): 14/14 PASS。B7/B8 excluded |
| 总体判断 | 相比原 redteam 明显改善。002 从 partial-credible 升级为 credible (Loop 7, ab013ed — D04/D05/D06 全部关闭，12/12 PASS)。003 real provider code path verified with evidence，仍存在 prompt-steering、单 tool、单次运行等 caveats，标记为 partial-credible。008 evidence chain fully closed, **最后一个 caveat 已关闭 (v3, 2026-05-31)**: `core.chat()` → ActionPlan schema enforcement working → real model stable JSON → scheduler → 14/14 PASS, 0 MODEL_BEHAVIOR_CONCERN；not product-ready；B7/B8 大型架构/产品化决策，不进入当前阶段 |
| REAL-EVIDENCE closure credibility | **8/8 evidence collected**。7/8 credible (001/002/004/005/006/007/008)；**1/8 partial-credible / credible-with-caveats** (003 code-path credible with blocking demonstrated, prompt-steered single-tool adversarial caveats) |
| 核心 runtime milestone | MAIN-PATH VALIDATED；002 real provider E2E 验证通过 (12/12 PASS, ab013ed) — **credible** with scope caveats (prompt-steered, single-skill)；003 real provider E2E 验证通过 — code-path credible with blocking demonstrated, partial-credible (prompt-steering, single-tool, single-run)；006/007/008 credible |
| 明确排除 | B7 Multi-instance readiness；B8 TUI architecture；003 标记为 partial-credible / credible-with-caveats；not product-ready |

主要纠偏点：
- `RuntimeDecisionFrame` registry 当前仍没有 READY branch point，状态文档不能把它当作完全 READY 事实源。
- Skill allowed_tools 的 contract path 有效；002 real provider dogfood 已验证 code path (12/12 PASS, ab013ed) — 标记为 credible（remaining scope caveats: prompt-steered, single-skill）。003 real provider dogfood 已验证 (5/5 PASS) — 标记为 partial-credible（prompt-steering, single-tool, single-run caveats）。
...
| REAL-EVIDENCE-003 | Skill allowed_tools | **credible-with-caveats** | lifecycle→mediator→gate enforcement 全链闭合。Multiple disallowed business tools (read_file/read_file_lines/SKILL_SELECT) blocked via skill_allowed_tools→rejected。active_skill_id evidence 完整性已验证 (R36 PASS)。**Production code fix (2026-05-31)**: mediator._route_gate() 动态读取 active_skill_id → gate_payload；ToolGateHandler rejected+allowed 双路径 evidence_extra 含 active_skill_id。**10 PASS / 0 FAIL / 7 CONCERN / 0 SKIP** (AnthropicCompatibleProvider, `scripts/real_evidence_003_hardening.py`)。27 focused tests PASS。**Caveats**: (1) prompt-steered adversarial prompts; (2) request_user_input 模型主动规避 (defense-in-depth); (3) 单 skill 场景; (4) H2 对抗风格全部 CONCERN (强模型自主规避限制 gate-only path 测试覆盖率)。Caveats 不影响核心声明——gate 正确拒绝了所有到达 gate 的 disallowed tool requests。 |
...
**保守证据可信度基线 (2026-05-31)**: 8/8 evidence collected。7/8 credible (001/002/004/005/006/007/008)；1/8 partial-credible / credible-with-caveats (003 code-path credible with blocking demonstrated, prompt-steered single-tool adversarial caveats)。002 real provider SKILL_SELECT 12/12 PASS；003 real provider disallowed-tool blocking 5/5 PASS。建议阶段性收口。B7/B8 不进入当前阶段。Not product-ready。
