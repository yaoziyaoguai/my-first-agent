...
- After any `docs/current/` reorg or file move (e.g. archiving a roadmap doc,
  adding a new current doc), run `graphify update . --force` so `graph.json`
  does not keep stale current-file references (G-006). Dirty/refused updates:
  pass `--force` only when graphify reports missing chunk files from a prior
  session and the existing graph is known-stale.
