"""_safe_noop: 安全空操作工具——仅用于 ToolRegistry gate 端到端验证。

中文学习边界：
_safe_noop 是唯一允许通过 `_` 前缀 gate 限制的内部 safe no-op anchor 工具。
它以 `_` 下划线开头，因此 get_model_visible_tools() 会将其排除（模型不可见），
但 ToolGateHandler 通过最小 allowlist 仍可在 TOOL_REGISTRY 中查到它。

为什么存在：
- ToolRegistry Safe Tool Anchor E2E 需要一个零副作用的生产工具来验证 gate 路径
