...
**最后更新**: 2026-05-30 (002/003 real provider evidence collected — 002 model-owned SKILL_SELECT 6/6 PASS + 003 disallowed-tool blocking 5/5 PASS with real AnthropicCompatibleProvider; 8/8 evidence collected, 5/8 credible + 3/8 partial-credible)
...
| REAL-EVIDENCE-003 | Skill allowed_tools | **partial-credible / code-path credible with blocking demonstrated** | Real provider disallowed-tool blocking 5/5 PASS。**Caveats**: (1) adversarial prompt 极度 prompt-steered: 直接要求调用 SKILL_SELECT 并设置 blog-writing; (2) 被阻断工具是 SKILL_SELECT 自身，非更自然的 disallowed 业务工具; (3) policy_path/rejection_reason 在 handler payload 中不在 RuntimeActionEvent.evidence，当前用 decision=rejected + skill_allowed_tools present 作为语义等价代理; (4) 单 skill/单工具/单 adversarial prompt 场景。 |
