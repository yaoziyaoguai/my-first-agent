...
- slash command 已开始结构化：`classify_user_input(...)` 会把 `/status now` 这类
  输入归为 `slash_command`，并通过 `parse_slash_command(...)` 生成
  `command_name` / `command_args` / `is_exit_command` metadata。这个 metadata 只是
  UI/control 输入语义，不是 RuntimeEvent，不是 checkpoint schema，也不是模型
  `conversation.messages`；`main.py::handle_slash_command(...)` 仍是命令执行点。
  当前只有 `/reload_skills` 是已实现命令，未知 slash 仍按 raw text 交回 Runtime。
...
- **InputIntent 边界**：InputIntent 是 UI Adapter -> Runtime 的输入归一层，
  ...
  pending 请求的答复，不是普通新任务。后续如果正式化 command registry 或调整 pending
  状态下的 slash 打断策略，必须在 adapter/Runtime 边界单独设计，不能把
  runtime_observer、debug print、terminal observer log、TaskState schema 或
  Anthropic API messages 混进来。
