...
### P1: Case K max loop — FakeProvider 中文关键词误匹配 (FIXED)
...
### P2: Case C tool_requested 误触发 — `request_user_input` 关键词重叠 (FIXED)
...
- **效果**: Case C 不再触发 tool_requested；echo_task_summary 的合法匹配（"查看当前任务摘要"）不受影响
- **Commit**: 本轮

### P3: Case D 长消息中英混合标点 tokenization 问题 (KNOWN, NOT FIXED)
...
2. **max loop guard** 被验证为安全阀而非成功条件
3. **每次 commit 前自动跑** — 回归由 CI/harness 负责，不依赖人类记忆
4. 用户只需手工做**主观体验判断**（文案是否顺手、UX 是否流畅），不再逐句跑基础回归
