...
| Filesystem Store | `agent/memory_fs_store.py` | ✅ | Store | frontmatter + index metadata 已覆盖 episodic/semantic/procedural dogfood；grouped Markdown write 与 index write 使用 locked atomic RMW；rejected 不写正式 record |
| Memory Runtime | `agent/memory_runtime.py` | 🟡 | Governance | W1/W2 runtime 保持薄编排；snapshot/recall current minimum 已满足，更精细 ranking 仍属 quality hardening |
| Operation Intent/Audit | `agent/memory_operations.py` | ✅ | Store | memory metadata 已随 intent 传递到 store |
| Snapshot Generator | `agent/memory_snapshot_generator.py` | ✅ | Recall | current minimum 已实现：scope/sensitive 过滤、T2 标注与上限、per-item/total budget 硬截断；更精细 ranking quality 属 dogfood hardening |
...
| Consolidation Domain | `agent/memory_consolidation.py` | ✅ | Consolidation | ConsolidationType 已包含 preference_evolved；deterministic foundation 在 detector 层实现 |
| Consolidation Detector | `agent/memory_consolidation_engine.py` | 🟡 | Consolidation | deterministic / keyword-level；preference_evolved foundation 已实现；复杂隐式演进 quality 需更多 dogfood |
| Consolidation Loader/Pipeline | `agent/memory_consolidation_loader.py`, `agent/memory_consolidation_pipeline.py` | ✅ | Consolidation | dry-run preview 已实现：只返回 candidates / would_dispatch_count / warnings，不写 `_pending` 或 store |
...
| Recall | ✅ | list_records/recall by scope/type/max_items/recency 与 current minimum snapshot budget governance 已实现；更精细 ranking quality 属 dogfood hardening |
...
- Filesystem grouped Markdown write 与 derived `_meta/index.json` 更新已加锁并使用唯一 temp file + atomic replace，避免并发 read-modify-write TOCTOU 覆盖。
- Phase 6 consolidation candidate 只进入 T1 pending review；dry-run 只预览 would_dispatch，不写 `_pending`；accept/edit-and-accept 后才写 semantic record，reject 不写。
...
- Phase 7 runtime hook summary 显示 active_records_count / min_active_records / gate_reason / disabled_reason，便于 dogfood 判断 fail-closed 原因。
...
### 14.6 Memory RFC Completion Checklist

**Current mainline completion state**：

- explicit retain / forget / update / session_only governance ✅
- filesystem store ✅ — filesystem-first 仍是当前 source-of-truth 和 reference implementation；index 是派生数据
- episodic T2 auto-retain ✅ — 仅 episodic，snapshot 中标注 `[自动记录]`
- review / pending_review ✅ — accept/edit-and-accept 后才写正式 store，reject 不写
- inline_confirmation ✅ — Agent loop adapter 已接入，reject/other/no response 不写；完整 TUI/CLI timeout 事件仍属体验 dogfood
- semantic consolidation ✅ — deterministic detector、loader、pipeline、T1 pending review、runtime hook 已实现
- consolidation dry-run ✅ — dogfood/audit preview，不写 `_pending`、不写正式 store、不 auto approve
- LLM-assisted consolidation dogfood ✅ — opt-in、脱敏、候选只进 T1 pending，不直接写 store
- preference_evolved deterministic foundation ✅ — semantic-only，explicit time-evolution marker + created_at order，T1 pending
- procedural emergence foundation ✅ — active_records >50 gate、pending_review、inline seam、runtime hook；不 silent retain，不 auto approve
- Phase 7 runtime hook ✅ — env-gated、fail-closed、dispatch-only
- fake / filesystem / real LLM dogfood harness ✅
- snapshot / recall current minimum ✅ — `recall()` 支持 scope/type/max_items/recency；snapshot 强制 scope/sensitive/budget/T2 governance
- secret handling ✅ — 不读取 `.env` 内容，不打印 secret；真实 sessions/runs 和 `agent_log.jsonl` 不作为 dogfood 输入
- filesystem write hardening ✅ — Markdown section 与 index write 使用 locked atomic RMW，保持 filesystem-first

**Dogfood / quality hardening**：

- true active records >50 后的 emergence quality observation
- real procedural emergence quality
- complex implicit preference evolution
...
Filesystem-first is current source-of-truth and reference implementation. Backend-independent semantics means lifecycle/governance rules do not depend on a backend; it does **not** mean implementing backend abstraction now.

### 14.7 Runtime Growth Constraint
...
- 当前遗留工作聚焦：dogfood maturity、complex implicit preference evolution hardening、inline_confirmation 真实 runtime dogfood；backend abstraction / vector DB / graph / embedding 保持 deferred
...
- LLM-assisted consolidation content generation — 🟡 interface + fake tests + real LLM dogfood 已实现（opt-in via `MEMORY_CONSOLIDATION_LLM_ENABLED`，默认 deterministic，增强 content/evidence_summary，fail-closed validation，不改变 governance/confidence/source_evidence）。Real LLM dogfood 已验证：project .env provider config, 2/2 candidates enhanced, governance all_pass
- LLM content generation validator — 🟡 已实现（hallucinated record_id 拦截、procedural-like content 拦截、N≥3 门槛、evidence_summary 长度检查）
- Contradiction handling — 🟡 keyword-level 已实现（opposing-marker pairs, clarification_needed candidate, consistency discount 0.7, deterministic, T1-only）
- Preference_evolved engine — 🟡 deterministic foundation implemented（explicit time-evolution markers + created_at order；semantic-only；T1 pending；复杂隐式偏好演进仍需 future dogfood hardening）
...
- `agent/memory_consolidation_loader.py:load_episodic_evidence()` — 只读装载函数
- 从 FilesystemMemoryStore 读取 episodic 记录，转换为 EpisodicEvidence
- 过滤规则：非 episodic / rejected / pending / session_only / ephemeral / sensitive_redacted / 空 content
- `SourceEvidenceLoadResult` — evidence + skipped_count + warnings
- 保留 confidence / created_at / tags / scope 等 metadata
- 只读：不调用 store 任何写方法，不修改 index.json，不创建新文件
- 不调 detector，不调 LLM，不接 runtime
- 39 个边界测试（`tests/test_memory_consolidation_loader.py`）
- `memory_fs_store._record_from_frontmatter`: 新增 tags 到 metadata 传播（最小修复）

**Consolidation Pipeline（已实现）**:
- `agent/memory_consolidation_pipeline.py:run_consolidation_pipeline()` — 串联 loader → detector
- 只读操作：不调用 store 写方法，不 import runtime / confirmation / policy / pending review / LLM 模块
- `ConsolidationPipelineResult` — frozen dataclass（candidates, evidence_count, skipped_count, warnings, detector_name）
- `_validate_candidate()` — defense-in-depth 校验层（fail-closed）：
  - 验证 memory_type=semantic、governance_route=T1、confidence∈[0,1]、source_evidence≥3、content 非空
  - 无效 candidate 丢弃并附加 warning，不崩溃
...
- 复用现有 anthropic SDK 和 config（API key / model name / base URL）；dogfood harness 不手工读取 `.env` 内容
- 53 个测试（`tests/test_memory_consolidation_llm.py`）
- Real provider auto-load dogfood 已执行：provider configured=yes，latest run blocked at auth_failed；candidate_count=2，T1 pending=2，direct semantic/procedural store write=0，secret output sanitized
- contradiction handling keyword-level 已实现（opposing-marker detection, clarification_needed candidate）
- preference_evolved deterministic foundation 已实现（semantic-only, T1 pending；LLM-assisted evolved preference quality 尚未完整 dogfood）
...
- **Runtime hook（opt-in）** — `agent.memory._maybe_run_emergence()` 由 session-end extraction 后调用，需 `MEMORY_EMERGENCE_ENABLED=true`，默认关闭
- **Fail-closed gates** — active_records <50 或 correction evidence <3 时不产生 candidate / 不写 pending
- **Non-interactive default** — runtime hook 默认 `confirmation_form="pending_review"`，只写 `_pending/`，不写正式 procedural store
...
- Foundation + deterministic E2E regression tests：
  ...
  - runtime hook default disabled → env enabled → active_records/correction evidence gate → pending_review dispatch
...
- **Remaining / deferred**: 完整 TUI/CLI timeout 事件接入、real emergence quality dogfood、complex implicit preference evolution hardening 属 dogfood/quality；backend abstraction 属 future deferred；不进入 DB / graph / embedding / vector store。
