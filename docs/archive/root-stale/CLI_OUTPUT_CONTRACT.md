...
- 阶段标签（默认 `Runtime v0.3 basic CLI shell`）
...
- 用法提示（v0.3 M3 起：`quit` + 一句 Skill 实验性提示；**不**再印
  `/reload_skills`——该 slash command 历史上无 handler，详见
  `docs/V0_3_SKILL_SYSTEM_STATUS.md`；同时提示 `python main.py health` /
  `python main.py logs --tail 50` 作为维护入口）
...
`[status] state=<user-facing-state> · status=<runtime-status> · step=<i>/<n> · current_step=<title> · pending_tool=<name> · msgs=<count>`

`state=` 必须使用用户可读交互状态：planning / awaiting confirmation /
executing tool/model / waiting user input / finished / failed。`status=` 保留
Runtime 内部状态名，方便与 checkpoint/logs 对照。只在状态变化点打一次，
**不**做 inplace 重绘 / 定时刷新（避免 curses 依赖）。
...
## 13. v0.3 M2/M3/M4 · 子命令与文案完成态（增量补充）

> 本节冻结 v0.3 M1 之后 M2-M4 落地的子命令、用户可见入口与文案边界。
> 任何破坏这些约束的 PR 都会被 `tests/test_v0_3_shell_completeness.py`
> 拦下。

### 13.1 子命令清单（必须存在且 main.py 已 wire）

| 命令 | 行为 |
|---|---|
| `python main.py` | 进入主对话循环，启动屏走 §12.1 |
| `python main.py --shell` | 显式进入同一个基础 CLI Shell（与无参数等价） |
| `python main.py health` | M2 结构化健康报告（人类可读） |
| `python main.py health --json` | M2 机器可读 JSON（schema 见 `agent/health_report.py`） |
| `python main.py logs` | M4 日志摘要（默认 tail 50 + 隐藏 runtime_observer） |
| `python main.py logs --tail N` | M4 截尾条数 |
| `python main.py logs --session <id-prefix>` | M4 按 session 过滤 |
| `python main.py logs --event <type>` | M4 按事件类型过滤 |
| `python main.py logs --tool <name>` | M4 按工具名过滤 |
| `python main.py logs --include-observer` | M4 显式打开极噪事件 |

### 13.2 启动屏 Skill 文案（M3）

启动屏**必须**含「实验性」字样并指向 `docs/V0_3_SKILL_SYSTEM_STATUS.md`，
**不允许**重新印 `/reload_skills` 或任何暗示 Skill 已成熟的字符串。

### 13.3 logs viewer 输出契约（M4）

详见 `docs/V0_3_OBSERVER_LOGS.md`。本节只锁两条不变量：
- 单行输出**不得**包含原始 content / result / system_prompt 正文
- 兜底 `mask_secrets` 命中时显示 `[REDACTED]`，不得静默放行
