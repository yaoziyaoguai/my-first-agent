...
| 2 | 工具失败 | `读取不存在的文件` | `tool.failed` + `执行失败。` + 具体错误，**不再误报「执行完成」** |
| 3 | 项目外路径写入 | `写入 ~/v0_2_outside_test.txt` | 即使用户确认 y，依旧被 `pre_write_check` 拒绝，显示 `已被工具内部安全检查拒绝。` |
| 4 | 敏感文件读取 | `读取 /tmp/server.pem` | `tool.rejected` + `被安全策略拒绝：...`，**不再误报「用户连续拒绝」**，也不泄漏文件内容 |
| 5 | 用户拒绝 | write_file 触发确认时回 `n` | `tool.user_rejected` + `用户拒绝执行，已跳过。` |

## 3. 仍保留的非阻塞观察项

以下项目在本轮人工验收中观察到，**不属于 v0.2 RC blocking**，留给后续 milestone：

- **agent_log.jsonl 体量**：当前 80MB+ / 18w+ 行，未发现真实 secret 泄漏
  （`grep sk-ant-[A-Za-z0-9_]{20,}` = 0），但需要后续引入 size 健康检查 / 日志轮转。
- **sessions/ 累积**：sessions 目录已 100+ 快照，全部在 `.gitignore` 内，
  暂无清理策略；后续可加 retention。
- **workspace_lint 健康检查 warning**：启动时偶有 warning，不影响主流程，
  非 RC blocking。
- **Skill 提示文案偏强**：当前 system prompt 中 Skill 相关引导措辞偏强势，
  容易让模型反复尝试调用，可在后续微调，不影响 v0.2。
