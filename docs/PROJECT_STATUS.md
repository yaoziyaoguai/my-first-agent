...
**最后更新**: 2026-05-31 (002 Skill Selection Plan 3 Phase 4-7 完成：ActiveSkillLifecycle + ToolRuntimeMediator allowed_tools integration + real provider Plan 3 dogfood + B7 extension points。**74 focused tests PASS**。**Phase 6 dogfood: 9 PASS / 3 CONCERN / 0 FAIL** with real AnthropicCompatibleProvider。Plan 3 pipeline end-to-end verified: turn-start selection → model SKILL_SELECT → lifecycle.activate() → allowed_tools → TOOL_GATE enforcement → evidence chain。)
**状态**: 阶段性收口。Score 4.1/5 conservative baseline with caveats。Evidence collected: 8/8。Credible: 6/8 (001/004/005/006/007/008)。Partial-credible: 2/8 (002/003, see caveats below)。002: **partial-credible / Plan 3 full pipeline verified with real-model evidence** — Plan 3 (Phase 3-7) completed: turn-start structured selection + ActiveSkillLifecycle + allowed_tools from lifecycle → ToolRuntimeMediator → TOOL_GATE enforcement + B7 extension points。Real provider Plan 3 dogfood: 9 PASS / 3 CONCERN / 0 FAIL (D01-D08 + P3S9-S12)。74 focused tests PASS (26 lifecycle unit + 15 turn-start selection + 8 allowed_tools lifecycle + 4 registry B7 + 21 registry baseline)。**Production code**: ActiveSkillLifecycle 双写 (lifecycle.activate() + backward-compat dict)；core.py / response_handlers.py / skill_tool.py lifecycle integration；SkillRegistry.scope B7 extension point。**Caveats**: (1) prompt-steered: 输入含 "demo"，模型选择 demo-note-maker 是合理预期，非自然对话自主发现; (2) 单 skill 场景: 未覆盖多 skill 竞争，需 chat() 架构改进; (3) evidence 事件粒度: selection.entered / candidates.built 未作为独立事件 dispatched (信息已在 skill.select 中); (4) 中文关键词匹配弱 (已知限制)。003: **partial-credible / code-path credible with blocking demonstrated** — real SKILL_SELECT activates demo-note-maker → adversarial prompt 诱导 SKILL_SELECT("blog-writing") → TOOL_GATE rejects via skill_allowed_tools→rejected → no TOOL_INVOKE → FORCE_STOP (5/5 PASS)。**Caveats**: (1) adversarial prompt 极度 prompt-steered: 直接要求调用 SKILL_SELECT 并设置 blog-writing; (2) 被阻断工具是 SKILL_SELECT 自身，非更自然的 disallowed 业务工具; (3) policy_path/rejection_reason 在 handler payload 中不在 RuntimeActionEvent.evidence，当前用 decision=rejected + skill_allowed_tools present 作为语义等价代理; (4) 单 skill/单工具/单 adversarial prompt 场景。006: **credible — 完整 evidence chain 闭合 (12/12 PASS)**。007: **credible — runtime-mediated execution chain 已验证** (10/10 PASS)。008: **credible — evidence chain fully closed, 最后一个 caveat 已关闭 (2026-05-31 v3)**: `core.chat()` → `_run_planning_phase()` ActionPlan schema enforcement working → real model outputs stable ActionPlan JSON → scheduler → 14/14 PASS。**不再有 MODEL_BEHAVIOR_CONCERN**。6 处 `Plan.model_validate` ActionPlan guard 全部就位。
