...
|-------------|------|---------|------|
| skill.select | PARTIAL | FAKE_LOCAL_USER_PATH | registry 已注入；fake 路径 auto-select；real 路径已增加确定性 keyword matching fallback（`agent/skill_selection.py`）——基于 user input 与 skill name/description/tags 的匹配，没有匹配时不选择；缺真实模型 SKILL_SELECT tool call（REAL-EVIDENCE-002/003） |
...
**Loop 3.2b 完成。** Child memory scope 已通过 parent ToolRuntimeMediator 中介：memory_scope=none/propose 区分、namespaced store write、SUBAGENT_CHILD_MEMORY_REQUEST evidence、24 个 contract tests 全部通过。code path complete, real provider dogfood pending (REAL-EVIDENCE-006)。
...
| Loop 2.2 | Skill Activation Main-Path Completion | **code path complete, real validation pending** — (1) body injection: SKILL_SELECT→body load→[Active Skill Instructions] 注入 prompt；(2) allowed_tools enforcement: ToolGateHandler 拦截非允许工具→rejected→FORCE_STOP→不进 execute_single_tool，ToolRuntimeMediator 传递 skill_allowed_tools；(3) **Loop 2.2 remediation (2026-05-28)**: 修复 real provider 路径下 model_decision_metadata 为空导致 SKILL_SELECT 总是返回 no_suitable_skill 的 blocker——新增 `agent/skill_selection.py` 确定性 keyword matching fallback（基于 user input 与 skill name/description/tags 匹配）；(4) 13 L2 contract + 6 L3 pipeline + 15 skill tool enforcement + 15 real provider selection tests 全部通过（49 tests）。剩余：真实模型 SKILL_SELECT tool call（REAL-EVIDENCE-002）、real dogfood E2E（REAL-EVIDENCE-003）已登记为 validation debt |
