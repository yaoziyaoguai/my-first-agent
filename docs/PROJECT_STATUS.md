...
**最后更新**: 2026-05-31 (008 v2 dogfood: 5 PASS / 1 MODEL_BEHAVIOR_CONCERN — core.chat() path 结构正确但模型不输出 ActionPlan schema JSON; _run_planning_phase() unified to single API call + ActionPlan/Legacy dual parsing; P1/P4/P5 closed)
**状态**: 阶段性收口。Score 4.0/5 conservative baseline with caveats。Evidence collected: 8/8。Credible: 5/8 (001/004/005/006/007/008)。Partial-credible / credible-with-caveats: 3/8 (002/003, see caveats below)。002: **partial-credible / code-path credible with real-model evidence** — AnthropicCompatibleProvider emits tool_use("SKILL_SELECT") → ToolRuntimeMediator pipeline (gate/invoke/result) → _active_skill populated (demo-note-maker, body_len=300, allowed_tools) → model-owned path confirmed。Real provider evidence chain closed (6/6 PASS)。**Caveats**: (1) prompt-steered: 输入含 "demo"，模型选择 demo-note-maker 是合理预期，非自然对话自主发现; (2) 单 skill 场景: 未覆盖多 skill 竞争/未匹配/visible_skills 为空; (3) 单次 dogfood: 无统计稳定性; (4) results.json 缺 R1 record (不影响 core evidence chain 但降低完整性)。003: **partial-credible / code-path credible with blocking demonstrated** — real SKILL_SELECT activates demo-note-maker → adversarial prompt 诱导 SKILL_SELECT("blog-writing") → TOOL_GATE rejects via skill_allowed_tools→rejected → no TOOL_INVOKE → FORCE_STOP (5/5 PASS)。**Caveats**: (1) adversarial prompt 极度 prompt-steered: 直接要求调用 SKILL_SELECT 并设置 blog-writing; (2) 被阻断工具是 SKILL_SELECT 自身，非更自然的 disallowed 业务工具; (3) policy_path/rejection_reason 在 handler payload 中不在 RuntimeActionEvent.evidence，当前用 decision=rejected + skill_allowed_tools present 作为语义等价代理; (4) 单 skill/单工具/单 adversarial prompt 场景。006: **credible — 完整 evidence chain 闭合 (12/12 PASS)**。007: **credible — runtime-mediated execution chain 已验证** (10/10 PASS)。008: **credible — evidence chain fully closed**: (A) `_run_main_loop(action_scheduler=...)` E2E (10/10 PASS) + (B) `build_action_plan_from_model_output()` bridge (7/7 tests) + (C) v1 model plan validation (13/13 PASS via `provider.create()`) + v2 core.chat() path (5 PASS / 1 MODEL_BEHAVIOR_CONCERN — 结构正确但模型输出非 ActionPlan schema)。P1/P4/P5 闭合：`generate_action_plan()` 进入主 runtime path、`load_plan()` 不再静默吞错、evidence payload 补齐。
...
| Loop 2.2 Skill Activation / allowed_tools | PARTIAL | 3 | model-owned SKILL_SELECT 工具路径已集成；002 real provider SKILL_SELECT 验证通过 (6/6 PASS) 但 partial-credible (prompt-steered single-skill single-run caveats)；003 real provider disallowed-tool blocking 验证通过 (5/5 PASS) 但 partial-credible (prompt-steered single-tool adversarial caveats) |
...
| Loop 3.4 Advanced Scheduler | VALIDATED | 4 | 008 credible — Gap A: `_run_main_loop(action_scheduler=...)` E2E (10/10 PASS) + Gap B: `build_action_plan_from_model_output()` bridge (7/7 tests) + Gap C v1: real provider model-generated JSON → bridge → scheduler → evidence (13/13 PASS) + v2 core.chat() path (5 PASS / 1 MODEL_BEHAVIOR_CONCERN)；27/27 scheduler tests pass；evidence chain fully closed；P1/P4/P5 闭合 |
...
| REAL-EVIDENCE-002 | Skill selection | **partial-credible / code-path credible with real-model evidence** | Real provider (AnthropicCompatibleProvider) SKILL_SELECT 模型自主调用 6/6 PASS (prompt-steered)。**002 hardening (2026-05-30)**: 非引导式 prompt + 多 skill 竞争 + 无匹配 skill + 空 visible_skills + 稳定性验证 (9 PASS / 3 FAIL / 5 CONCERN)。确认了：(a) 非引导式 prompt 下模型绕过 SKILL_SELECT 直接调用工具 → handler no_suitable_skill；(b) select_skill_for_real_provider() 中文关键词匹配弱；(c) 稳定性 x3 一致；(d) chat() 架构限制无法注入多 skill registry。**Caveats**: (1) prompt-steered: 输入含 "demo"，hardening 确认非引导式 prompt 触发 no_suitable_skill; (2) 单 skill 场景: 未覆盖多 skill 竞争/未匹配/visible_skills 为空（架构限制）; (3) 稳定性已验证 x3; (4) results.json 缺 R1 record; (5) NEW: 中文关键词匹配弱。13 unit + 15 integration tests + 6 validation PASS。Hardening: scripts/real_evidence_002_hardening.py。 |
...
| REAL-EVIDENCE-008 | Advanced scheduler | **credible** (evidence chain fully closed) | Gap A: `_run_main_loop(action_scheduler=...)` E2E evidence chain 闭合 (10/10 PASS, V1-V10) — scheduler 通过完整 main runtime injection chain 进入 preprocessing block 并产生全部 5 种 business evidence + condition_flags + halt + topological order + backward compat。Gap B: `build_action_plan_from_model_output()` JSON→ActionPlan bridge (~50 lines, 7/7 tests PASS) — code fence 剥离、无效 node 跳过、空 nodes ValueError、多余字段容忍、无效 recovery fallback。**Gap C v1 (Model Plan via provider.create())**: 13/13 PASS — real AnthropicCompatibleProvider 通过 `provider.create()` + custom system prompt 生成合法 JSON ActionPlan → bridge → scheduler → evidence chain complete。**v2 (core.chat() path, 2026-05-31)**: 5 PASS / 1 MODEL_BEHAVIOR_CONCERN — `core.chat()` → `_run_planning_phase()` → `generate_action_plan()` 路径正确，但模型输出 JSON 不符合 ActionPlan schema（使用 `steps` 而非 `nodes`、`tool` 而非 `action_type`/`target`、缺少 `steps_estimate`）。`_run_planning_phase()` 正确识别为单步并跳过。Safety checks (M10-M13) 全部通过。**P1/P4/P5 闭合**: (P1) `generate_action_plan()` 通过 `clean_text=` 委托进入主 runtime path，不再是孤岛；(P4) `load_plan()` 不再静默吞错；(P5) evidence payload total_nodes/entry_node_id/completed_counts 不再为 None。27/27 scheduler tests + 121 focused tests PASS。**剩余 caveat**: `core.chat()` 系统 prompt 覆盖 `ACTION_PLAN_PROMPT` JSON schema 指令 → 模型输出内部格式而非 ActionPlan schema。完整 `core.chat()` → model JSON → scheduler 闭环需 planner.generate_plan() 连接（B7/B8 范围）。 |

**保守证据可信度基线 (2026-05-30)**: 8/8 evidence collected。5/8 credible (001/004/005/006/007/008)；3/8 partial-credible / credible-with-caveats (002 code-path credible with real-model evidence, prompt-steered single-skill single-run caveats; 003 code-path credible with blocking demonstrated, prompt-steered single-tool adversarial caveats)。002 real provider SKILL_SELECT 6/6 PASS；003 real provider disallowed-tool blocking 5/5 PASS。建议阶段性收口。B7/B8 不进入当前阶段。Not product-ready。
