...
Skill v1 已经按这个 seam 实现：显式 trust root 经 `agent.skill.catalog` 冻结为不可变 descriptor/digest，`agent.skill.tools` 把每个 Skill 暴露为 `skill__<name>` READ_ONLY activation 工具加共享 `skill__read_resource`，`agent.composition.build_tool_registrations` 把它们与文件工具拼接进唯一 `KernelToolRuntime`。它没有 prompt hook、脚本执行、远程 registry 或自动激活。
