...
Implement `scripts/materialize_020a_test_runtime.py` as a tracked stdlib-only helper. It requires explicit `--source-root`, `--destination-root`, and one or more repeated `--protected-root` values; it never defaults or infers any of those paths from cwd, module location, environment or user state. The Python API mirrors this with a required keyword-only non-empty `protected_roots` tuple. `source_root` must first pass `qualify_hermetic_runtime_closure`. Open every explicit locator component-by-component with `O_DIRECTORY|O_NOFOLLOW`; do not call `resolve()` before symlink admission. Before creating the destination, pin the source, destination parent and every protected root, then reject source/destination overlap with each other or with any product/workspace/state protection domain. After `mkdir`, capture the no-follow destination entry identity, require the opened directory fd to match it, and reject that fd if it aliases the pinned source, destination parent or any protected root; no copied byte may be written before these checks pass. Copy only the exact manifest plus `closure.inventory` paths with descriptor-relative `O_NOFOLLOW` reads and `O_CREAT|O_EXCL|O_NOFOLLOW` writes, preserving canonical `0444/0555` modes and verifying size/SHA-256 after each write. Write the already-canonical manifest last, fsync files/directories, then requalify the already-pinned destination tree and require the destination-parent entry to match that pinned inode both before and after qualification. A post-create failure closes every descriptor but deliberately leaves the partial destination for its caller-owned private temp root to reclaim; the materializer must not recursively `unlink`/`rmdir` names because POSIX deletion cannot atomically bind a name to the pinned inode and could delete a concurrent external replacement. The caller must supply a private verifier-owned destination parent and guarantee no concurrent same-UID mutation of that parent, the admitted source tree or any protected root for the duration of the call; this is the explicit limit of the path-returning v1 interface. The materializer must not enumerate protected-root contents merely to infer historical inode provenance. Unit tests use a synthetic non-sensitive qualified fixture; E2M/E3 pass an explicit application-release `skill-runtime-v1` root plus their verifier-owned product/workspace/state roots and materialize a fresh copy. The fresh wheel-verifier venv is never accepted as `source_root`.
