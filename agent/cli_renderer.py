...
def render_provider_mode_banner() -> str:
    ...
    优先读 config/config.yaml，再 fallback 到 legacy env。
    """
    import os
    ...
    try:
        ...
        if provider_type != "fake" and unified.source == "config_yaml":
            return f"[provider] mode={provider_type} (真实 API — model={model})"
    except Exception:
        pass
...
def render_onboarding() -> str:
    ...
    lines = [
        ...
        "",
        "  当前状态：",
        "    • 一页状态文档：docs/current/PRODUCT_CAPABILITY_AUDIT.md",
        "      （python main.py capability-status 查看能力真相表）",
        "    • fake/local rehearsal 只是工程验证，不等于产品可用性结论",
        "    • local trial 以 README 和 docs/current/OPERATOR_GUIDE.md 为当前入口",
        "    • real provider tool-name 协议已修复，interactive CLI 已验证",
        "    • 当前 provider 模式见上方 [provider] banner",
