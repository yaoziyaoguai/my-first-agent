...
**最后更新**: 2026-05-28 (Loop 2.3 — Storage/Checkpoint True Resume code path complete)
...
| Milestone | Commit | 简述 |
|-----------|--------|------|
| Loop 2.3: Storage/Checkpoint True Resume | — | **code path complete, real validation pending** — save/load 接入统一 main runtime path：(1) `core.py` 3 处 direct `save_checkpoint()` 调用迁入 `_dispatch_checkpoint_save()`，通过 `dispatcher.route_from_runtime_loop(CHECKPOINT_SAVE)` 产生 L3 evidence；(2) `session.py` resume 路径（pipe 模式 + TTY 模式）通过 `_try_dispatch_checkpoint_resume()` 记录 CHECKPOINT_RESUME evidence（dispatcher 按需构建，`_already_loaded=True` 跳过重复 load）；(3) `CheckpointSaveHandler`/`CheckpointResumeHandler` 在 `phase1_hook.py` 注册（共 15 个 handler）；(4) `RuntimeDecisionFrame` checkpoint.save/resume branch points 更新为 code path complete + REAL-EVIDENCE-004 引用；(5) 16 contract tests pass（4 save mediation + 5 resume mediation + 4 roundtrip + 2 not fakeable + 1 L3 hook-level）；(6) 已有 68 checkpoint tests 无回归。session.py 退出路径（finalize/interrupt）仍为 direct call——非主路径，不阻塞。真实 API/model roundtrip 验证已登记为 REAL-EVIDENCE-004 |
