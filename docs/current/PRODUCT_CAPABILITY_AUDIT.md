...
| Tool runtime and registry | `agent/tool_registry.py`, `tool_runtime_mediator.py`, `tool_executor.py`, `agent/tools/*`; golden tool tests | **L4 (narrow: write_file only)** | ~10 governed tools registered (write/edit/read/shell/web/memory/skill/demo/safe-noop). Run 12 proved `write_file` end-to-end. Mediator/executor correctness + evidence-only TOOL_INVOKE pinned by AST tests. | Only `write_file` is real-proven. `run_shell`/`fetch_url` have zero real evidence. Broad tool catalog docs/status incomplete. | Dangerous tools (shell/web) becoming risky if broadened without confirmation/path-safety dogfood. | Keep mediator/executor spine fixed; document the real-safe tool subset and a per-tool confirmation matrix. |
...
| Security / config diagnostics | `security.py`, `health_check.py`, `health_report.py`, `provider/diagnostics.py`, `scripts/check_*`; ~14 test files | **L4 (soft)** | R-series ran with no secret leak observed. Status api_key redaction verified SYNTHETICALLY (R-G01, synthetic key). AST boundaries forbid non-provider modules importing SDKs. Config examples must use `sk-REPLACE_ME`. | R-004 (real key in real `status`) is explicitly DEFERRED (`R_TRIAL_RUN_LOG.md:190-191`). Redaction is contract-proven, not real-secret-proven. No live-secret redaction smoke. | Secret/config leakage if diagnostics start printing raw config/headers. | Extend status docs, never raw output; never stage `config/config.yaml` or `.env`. Do NOT claim redaction verified against real credentials. |
...
- Do not claim redaction is verified against real credentials; R-004 is deferred
  and the proof is synthetic + static.
