...
1. simple CLI 主循环可以端到端跑通"读 README、写中文总结到 summary.md"这类任务（手动 smoke 通过）
...
6. **最小 CLI/TUI 输出契约已冻结**（B2，本轮**未启动**）
...
| **B2** 冻结最小 CLI/TUI 输出契约 | ⏳ 未启动 | 与 B1 解耦，本轮**不启动** |
| **B3** 真实手动 smoke（需 `ANTHROPIC_API_KEY`） | ⏳ 未启动 | 依赖 B2，本轮**不启动** |

> v0.1 是否毕业：**否**。本轮只关 B1。B2 / B3 何时启动由用户决定，不在本文件范围。
