...
本文件最初记录 `v0.7.0` Tooling Foundation / MCP readiness milestone 的边界。
当前 HEAD 已进入 post-v0.8.0 safe-local / readiness / RFC-gated development：
MCP config management、external readiness dry-run 与安全边界文档已补齐；但这仍不是
完整 MCP 使用手册，也不声明已经支持外部 MCP server、真实 MCP endpoint、真实 secret
或生产 remote server auth。
...
## Historical release / dogfooding closure status
...
v0.7.0 milestone 只提供 loader seam；post-v0.8.0 remediation 后已有 safe fixture
config management / plan-first apply / readiness dry-run 证据，但仍只允许显式安全路径，
不读取真实 MCP config，不写用户 home config，也不连接真实 MCP server。
