...
| **Known limitation** | ~~TOOL_MEDIATOR_GAP resolved.~~ ~~MODEL_BEHAVIOR_CONCERN resolved — child_tools schema fix 使模型可见 tool schema，模型输出 API-native structured tool_use。~~ SimpleNamespace turn_state + _turn_context 私有属性访问 caveat 仍在（不影响功能正确性）。demo-stat-real memory_scope=none 故 child memory proposal path 未在 real provider E2E 中触发（contract tests 已覆盖）。 |
