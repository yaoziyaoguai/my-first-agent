...
- 只匹配 manifest 显式声明的 metadata——不做隐式跨语言翻译/猜测
- skill 用什么语言声明 metadata，就按什么语言匹配
- 中文 prompt 匹配中文 aliases/triggers/description；英文 prompt 匹配英文
"""
...
class SkillCandidateRetriever:
    ...
    def _score_by_triggers(
        ...
        只取最高分的单个匹配。
        不做跨语言匹配——manifest 中声明的触发器按原始语言匹配。
        """
        best_score = 0.0
        best_term = ""
    ...
    def _score_by_keywords(
        ...
        只匹配 manifest 显式声明的 metadata，不做跨语言猜测。
        """
