...
**最后更新**: 2026-05-31 (002 Plan 3 Loop 3 closeout: D02 candidates=0 fixed via SkillSelectionProbeHandler, D03 Chinese bigram matching, D05 skill_allowed_tools in evidence_extra, D06 negative_triggers, coverage tests added. 9 PASS / 3 CONCERN / 0 FAIL. **002 建议状态: credible-with-caveats**。剩余 D04/D05/D06 均为 MODEL_BEHAVIOR 或下游依赖，非 runtime integration gap。)
**状态**: 阶段性收口。Score 4.1/5 conservative baseline with caveats。Evidence collected: 8/8。Credible: 6/8 (001/004/005/006/007/008)。Credible-with-caveats: 1/8 (002)。Partial-credible: 1/8 (003, see caveats below)。002: **credible-with-caveats / Plan 3 pipeline verified with real-model evidence** — Plan 3 (Phase 2-7) complete: SkillCandidateRetriever + selection section (Phase 2-3, wired into production runtime via Loop 1 P0 fix aef2f8f, verified via Loop 2 dogfood re-run D01/D02 PASS) + ActiveSkillLifecycle (Phase 4) + allowed_tools from lifecycle → ToolRuntimeMediator → TOOL_GATE enforcement (Phase 5) + real provider dogfood (Phase 6, 9 PASS / 3 CONCERN / 0 FAIL) + B7 extension points (Phase 7)。**Production code**: SkillCandidateRetriever + build_skill_selection_section() in refresh_runtime_system_prompt()；ActiveSkillLifecycle 双写；core.py / response_handlers.py / skill_tool.py lifecycle integration；SkillRegistry.scope B7 extension point；SkillSelectionProbeHandler for selection.entered/candidates.built evidence (Loop 3 fix)。**Caveats**: (1) prompt-steered: 输入含 "demo" 时模型选择 demo-note-maker 是合理预期，非自然对话自主发现; (2) 单 skill 场景: 未覆盖多 skill 竞争，需 chat() 架构改进; (3) D04 CONCERN: skill.select present but lifecycle activation not captured in evidence — 下游 effect; (4) D05 CONCERN: TOOL_GATE payload field name mismatch — skill_allowed_tools now in evidence_extra, script assertion alignment pending; (5) D06 CONCERN: 模型对数学 prompt 触发 skill.select — MODEL_BEHAVIOR_CONCERN (negative_triggers 已添加到 retriever 层，但模型仍可能绕过)。**Loop 3 修复**: D01/D02→PASS (SkillSelectionProbeHandler)，中文 bigram 匹配，negative_triggers，coverage tests。003: **partial-credible / code-path credible with blocking demonstrated** — real SKILL_SELECT activates demo-note-maker → adversarial prompt 诱导 SKILL_SELECT("blog-writing") → TOOL_GATE rejects via skill_allowed_tools→rejected → no TOOL_INVOKE → FORCE_STOP (5/5 PASS)。**Caveats**: (1) adversarial prompt 极度 prompt-steered: 直接要求调用 SKILL_SELECT 并设置 blog-writing; (2) 被阻断工具是 SKILL_SELECT 自身，非更自然的 disallowed 业务工具; (3) policy_path/rejection_reason 在 handler payload 中不在 RuntimeActionEvent.evidence，当前用 decision=rejected + skill_allowed_tools present 作为语义等价代理; (4) 单 skill/单工具/单 adversarial prompt 场景。006: **credible — 完整 evidence chain 闭合 (12/12 PASS)**。007: **credible — runtime-mediated execution chain 已验证** (10/10 PASS)。008: **credible — evidence chain fully closed, 最后一个 caveat 已关闭 (2026-05-31 v3)**: `core.chat()` → `_run_planning_phase()` ActionPlan schema enforcement working → real model outputs stable ActionPlan JSON → scheduler → 14/14 PASS。**不再有 MODEL_BEHAVIOR_CONCERN**。6 处 `Plan.model_validate` ActionPlan guard 全部就位。
...
| REAL-EVIDENCE-002 | Skill selection | **credible-with-caveats / Plan 3 full pipeline verified with real-model evidence** | **Plan 3 Phase 3-7 (2026-05-31)**: Phase 3 turn-start structured selection (15 tests) + Phase 4 ActiveSkillLifecycle (26 tests) + Phase 5 lifecycle→mediator→gate allowed_tools (8 tests) + Phase 6 real provider Plan 3 dogfood (9 PASS / 3 CONCERN / 0 FAIL — D01-D08 + P3S9-S12) + Phase 7 B7 extension points (4 tests)。**74 focused tests PASS**。**Loop 3 (2026-05-31)**: D02 candidates=0 fixed (SkillSelectionProbeHandler)，D03 Chinese bigram matching，D06 negative_triggers，coverage tests。Phase 6 dogfood 验证: D01/D02 PASS, D03 model selection ✓, D04 lifecycle activation (CONCERN — downstream effect), D05 allowed_tools in TOOL_GATE (CONCERN — script alignment pending), D08 gate rejection ✓。Lifecycle cross-turn persistence ✓。**Caveats**: (1) prompt-steered: 输入含 "demo" 时正常，非引导式 prompt 触发 no_suitable_skill (已知); (2) 单 skill 场景: chat() 架构限制无法注入多 skill registry; (3) D04/D05/D06 CONCERN — 均为 MODEL_BEHAVIOR 或下游依赖，非 runtime integration gap。74 focused tests + 6 validation PASS。 |
