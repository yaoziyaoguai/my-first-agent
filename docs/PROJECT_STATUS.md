...
| REAL-EVIDENCE-002 | Skill selection | **partial-credible / code-path credible with real-model evidence** | Real provider (AnthropicCompatibleProvider) SKILL_SELECT 模型自主调用 6/6 PASS。**Caveats**: (1) prompt-steered: 输入含 "demo"，模型选择 demo-note-maker 是合理预期，非自然对话自主发现; (2) 单 skill 场景: 未覆盖多 skill 竞争/未匹配/visible_skills 为空; (3) 单次 dogfood: 无统计稳定性; (4) results.json 缺 R1 record (不影响 core evidence chain 但降低完整性)。13 unit + 15 integration tests + 6 validation PASS。 |
