...
输出链路上，`core.chat(..., on_runtime_event=...)` 现在是 Runtime 到 UI 的最小
用户可见输出边界。assistant streaming delta、DisplayEvent 和轻量工具生命周期
提示会先包装成 `RuntimeEvent`，再由 `main.py` / Textual adapter 投影到界面。
TUI 维护轻量 `conversation_history` 用于显示 `You` / `Assistant` / `Tool` /
`System`，长输出自动滚动到底部；`chat()` 的 final return 只作为非 streaming
fallback 或旧路径控制文案，不应把已 streaming 的正文再追加一遍。
...
只消费 RuntimeEvent 并渲染成 `Assistant` / `Tool` / `System` 消息，不读取
Runtime state，也不保存 checkpoint。

`RuntimeEvent` 的边界要特别清楚：

- 它是 Runtime -> UI 的用户可见输出流，只服务界面投影。
- 它不写 checkpoint，不进入 `conversation.messages`，模型不会在下一轮看到它。
- 它不是 `runtime_observer` debug event；debug/checkpoint 日志仍走结构化日志和
  显式 debug 开关。
...
本阶段开始引入轻量 `InputIntent` 输入分类层：

- `RuntimeEvent` 是输出边界，方向是 Runtime -> UI，用来投影用户可见输出。
- `InputIntent` 是输入边界，方向是 UI Adapter -> Runtime，用来把 raw text 初步归类为
  普通消息、slash command、plan/step/tool confirmation、request_user_input 回复、
  empty、exit、cancel 或 eof。
- `InputIntent` 只读 `raw_text` 和当前 `TaskState` 的 pending/awaiting 字段，不修改
  state，不触发 RuntimeEvent，不调用模型，不执行工具。
- `InputIntent` 不写 checkpoint，不进入 `conversation.messages`，不改变 Anthropic API
  messages，也不替代 TaskState；真正的 confirmation/request_user_input 状态推进仍由
  `core.chat()` 和 `confirm_handlers.py` 处理。
- Textual 产品路径和 simple CLI fallback 共享这层分类，避免 simple CLI 的历史
  `input()`/字符串协议继续散落到 TUI 主路径。
- plan / step / tool confirmation 的 yes/no/中文确认词表已收敛到
  `classify_confirmation_response(...)`。`confirm_handlers.py` 现在只消费
  `accept/reject/feedback` 分类结果并执行原有状态推进；它仍负责
  checkpoint 保存、语义 control event、pending_tool 清理和 tool_result placeholder，
  不能把 InputIntent 写入 messages，也不能改变 tool_use_id 配对。
- slash command 已进入轻量执行层：`classify_user_input(...)` 会把 `/status now`
  归为 `slash_command`，`parse_slash_command(...)` 生成 `command_name` /
  `command_args` / `is_exit_command` metadata；`agent.commands.CommandRegistry`
  消费这些 metadata 并返回 `CommandResult`。三者边界必须分开：InputIntent 是输入
  分类，CommandResult 是本地 UI/control command 结果，RuntimeEvent 是输出投影。
  它们都不能写 checkpoint 或 `conversation.messages`，也不能进入 Anthropic API
  messages、TaskState 状态机本体、tool_use_id 配对或 tool_result placeholder。
  当前集中支持 `/help`、`/status`、`/clear`、`/reload_skills`，`/exit` 仍由
  InputIntent 优先识别为退出控制输入；未知 slash command 由 registry 返回
  `command.result` 错误提示，不再交给模型。
- request_user_input reply 也已在输入边界上明确：当 `TaskState` 处于
  `awaiting_user_input` 且有 `pending_user_input_request` 时，普通文本会归为
  `request_user_reply`，随后仍由 `core.chat()` / `confirm_handlers.py` 投影成
  `user_replied` / `step_input` 语义。它不是普通新任务，不是 confirmation，也不是
  tool_result placeholder；tool_use_id 配对和 Anthropic API messages 不变。
- 模型协议投影边界也要分开看：`context_builder.build_execution_messages(...)`
  只把 `TaskState` 的当前 step、摘要和 `conversation.messages` 投影成 Anthropic
  API messages。`RuntimeEvent` 和 `InputIntent` 都不能进入这里。业务 tool_use /
  tool_result 的配对由 `_project_to_api(...)` 维护；`request_user_input` 是元工具控制
  信号，它的用户回复只作为 `step_input` 文本进入模型上下文，不生成 `ru_*`
  tool_result。
- 当前优先级仍按现有产品行为固化：`empty` / `exit` / `slash_command` 在 pending
  状态之前识别，因此 pending_user_input_request、pending_tool 或 plan confirmation
  期间 slash command 可以作为 UI/control 输入打断。是否禁止这种打断是后续产品决策，
  不能在输入分类层临时改状态机或 checkpoint。
...
- **stdout capture**：`main.py` 仍捕获 stdout 并过滤 debug/checkpoint 前缀，但它
  ...
  当 RuntimeEvent 已发出时，stdout 不再作为同轮 Textual completion 返回。本阶段又
  收窄了一层：`_run_textual_runtime_turn(...)` 调 core 时始终传 RuntimeEvent sink，
  即使调用方还提供旧 callback，也先在 main.py 转发。当前还不能完全删除 stdout
  capture，因为旧 fake/旧调用方和少量未事件化 print 仍需要在 Textual 中可见；删除
  条件是这些用户可见 print 都迁移为 RuntimeEvent 或明确不再投影到 TUI。
...
- **旧 callback**：`on_output_chunk` / `on_display_event` 仍保留给老调用方，但
  ...
  assistant delta / DisplayEvent 的兼容转发目标。Textual adapter 不再把旧 callback
  直接传给 core，而是把 RuntimeEvent 转发给旧 callback。新功能不应继续接入旧
  callback；长期应演进为 `chat_stream` / RuntimeEvent iterator，并逐步删除旧 callback。
...
- **tool lifecycle**：当前已有最小 DisplayEvent 覆盖
  `tool.awaiting_confirmation` / `tool.executing` / `tool.completed` /
  `tool.failed`，其中 `write_file` 确认会显示路径和内容 preview；模型开始规划
  工具调用时的 `tool.requested` 也已事件化。剩余债务是更丰富的工具结果摘要、
  失败详情结构化和完整 RuntimeEvent iterator。
- **checkpoint/debug hygiene**：checkpoint / runtime observer 已默认写结构化日志，
  只有 `MY_FIRST_AGENT_DEBUG=1` 时才打印 `[CHECKPOINT]` / `[RUNTIME_EVENT]` 等
  terminal 短日志。`main.py` 仍保留 prefix 过滤作为兼容防线；长期应由
  DisplayEvent 从源头隔离用户输出和内部调试。
- **Textual backend 边界**：Textual 只做 I/O，不耦合 Runtime state，不判断 y/n，
  不保存 checkpoint；任何新功能都应先确认没有把 Runtime 语义下沉到 TUI。
- **generation cancellation 边界**：当前 Esc 只属于 Textual 输入编辑层，语义是清空
  草稿并产生 `input.cancelled`，不是“取消正在生成”。生成中断需要一条完整的
  Runtime 生命周期链路：Textual 发起 cancel intent，main.py 持有并传递 cancel_token，
  `core.chat()` / streaming client 能 abort 模型流，Runtime 再通过
  `generation.cancelled` 或等价 RuntimeEvent 投影给 UI。这个能力不能靠在 Textual
  里停止 append chunk 的补丁实现，也不能把 RuntimeEvent、InputIntent、CommandResult、
  checkpoint、runtime_observer、conversation.messages、Anthropic API messages、
  TaskState、tool_use_id 配对或 simple CLI fallback 混在一起。删除当前 xfail 的条件
  是 cancel_token 和 generation lifecycle 均完成设计并有端到端测试。
- **CommandRegistry 边界**：`agent.commands` 是 UI/control command 执行层，只消费
  InputIntent metadata 并返回 CommandResult。它不是 RuntimeEvent、不是 InputIntent、
  不是 checkpoint schema，也不是模型上下文；不能写 `conversation.messages`，不能
  保存 checkpoint，不能推进 TaskState，不能承载 runtime_observer/debug 日志。
  `/clear` 只表达 UI 清屏意图，不 reset Runtime；`/status` 只读状态摘要。后续若要
  限制 pending 状态下哪些 command 可打断，必须单独做产品决策，而不是在 registry
  里偷偷读取 pending 状态改行为。
- **InputIntent 边界**：InputIntent 是 UI Adapter -> Runtime 的输入归一层，
  RuntimeEvent 是 Runtime -> UI 的输出边界，二者方向相反，不能混用。slash command
  是 UI/control 输入，不应进入模型消息或 checkpoint；request_user_input reply 是
  pending 请求的答复，不是普通新任务。后续如果调整 pending 状态下的 slash 打断策略，
  必须在 adapter/Runtime 边界单独设计，不能把 runtime_observer、debug print、
  terminal observer log、TaskState schema 或 Anthropic API messages 混进来。
- **request_user_input 语义债**：当前 `request_user_input` 是元工具控制信号，不写
  `conversation.messages` 里的 tool_use，也不会为用户回复生成 tool_result；用户回复
  通过 `user_replied` / `step_input` 投影给下一轮模型。这避免了悬空 tool_result，但
  也意味着它不是 Anthropic 工具配对协议的一部分。是否改成可配对 tool_result 语义
  需要单独决策，删除条件是设计清楚 tool_use_id 配对、checkpoint 兼容和 API messages
  migration；在此之前不能用 placeholder 补丁伪造 tool_result。
- **messages 投影边界**：`conversation.messages` 是内部事件流，Anthropic API
  messages 是投影结果。`_project_to_api(...)` 会为了业务工具协议合并 tool_result、
  删除夹在 tool_use/result 中间的控制事件，并只在业务 tool_use 缺失结果时补
  placeholder。这个 placeholder 不能套用到 `request_user_input` 元工具，也不能用来
  容纳 RuntimeEvent/InputIntent/checkpoint/debug 信息。
...
| `agent/display_events.py` | UI 投影事件定义与渲染；当前承载 RuntimeEvent、DisplayEvent、工具确认/生命周期提示，不进入模型上下文 |
...
| `agent/commands.py` | 轻量 slash CommandRegistry；执行 `/help` / `/status` / `/clear` / `/reload_skills`，返回 CommandResult，不写 messages/checkpoint |
| `main.py` | backend 选择 + 用户输入循环 + exit / slash command adapter + Ctrl+C 路由 + streaming bridge |
