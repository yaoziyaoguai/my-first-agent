...
def test_no_batch_memory_pending_memory_proposal_or_direct_write_from_v0_result() -> None:
    ...
    assert result.evidence["memory_runtime_direct_write"] is False
...
def test_v0_path_does_not_call_direct_memory_or_checkpoint_write_paths(
    ...
    monkeypatch.setattr(InMemoryMemoryStore, "apply_operation_intent", forbidden_write)
    monkeypatch.setattr(InMemoryMemoryStore, "store_retained_record", forbidden_write)
...
def test_parent_checkpoint_stores_safe_subagent_metadata_only() -> None:
    ...
    assert "RAW_CHILD_RESULT_SHOULD_NOT_LEAK" not in repr(result.evidence["checkpoint_metadata"])
