...
| `L3_HARDENING_TRIAGE.zh.md` | L3 hardening triage——8 模块逐模块 triage + recommended next target | current — triage completed; recommended next: Skill → L3 |
...
| `MEMORY_TAXONOMY_MAPPING.zh.md` | Memory taxonomy → MEM-2 mapping——user 三类 memory × source/type 双轴映射 | current — mapping completed;Memory L2;no emotion/agent-inferred path exists |
...
- **T-SKILL-L3 completed**: Skill System → L3, core-loop golden E2E evidence (`test_golden_skill_l3_core_loop.py` 2 passed).
