...
| **Status** | **CLOSED (partial-credible / code-path credible with real-model evidence)** — real provider SKILL_SELECT 模型自主调用验证通过。完整 ToolRuntimeMediator pipeline 证据链闭合。6/6 PASS（prompt-steered）。**002 hardening (2026-05-30)**: 非引导式 prompt 验证 9 PASS / 3 FAIL / 5 CONCERN — 确认了 prompt-steering 是真实限制（非引导式 prompt 下模型绕过 SKILL_SELECT 直接调用工具，handler 返回 no_suitable_skill）；select_skill_for_real_provider() 中文关键词匹配能力弱（纯中文 prompt 无法匹配 skill name 中的英文词）；稳定性 x3 一致（模型行为可复现）。**Caveats**: (1) prompt-steered — 输入含 "demo"，模型选择 demo-note-maker 是合理预期，非自然对话自主发现（hardening 确认：非引导式 prompt 触发 no_suitable_skill）; (2) 单 skill 场景 — 未覆盖多 skill 竞争/未匹配/visible_skills 为空（hardening 确认：chat() 内部构建 registry，架构限制无法注入多 skill）; (3) 单次 dogfood — 稳定性 x3 已验证（hardening: 3/3 一致）; (4) results.json 缺 R1 record (不影响 core evidence chain 但降低完整性); (5) NEW: select_skill_for_real_provider() 中文关键词匹配弱 — 纯中文 prompt 无法匹配英文 tag/name 中的关键词。 |
| **Blocking current code loop** | no |
| **Blocking READY claim** | no |
...
| **Credibility** | **partial-credible / code-path credible with real-model evidence** — real provider (AnthropicCompatibleProvider) 模型自主 tool_use("SKILL_SELECT") → ToolRuntimeMediator pipeline → _active_skill 填充 → model-owned path 确认（prompt-steered）。Code path + evidence chain 闭合，但非引导式 prompt 下模型绕过 SKILL_SELECT、中文关键词匹配弱、多 skill 竞争无法 E2E 测试等 caveats 仍存。自然对话中自主发现 skill 的能力未闭合。 |
