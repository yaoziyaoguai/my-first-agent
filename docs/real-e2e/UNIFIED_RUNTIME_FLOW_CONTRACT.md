...
项目工程流程（SDD → TDD → Implementation → Review → Debug 的迭代 loop、
review gate、回退规则、重试上限、升级条件和禁止模式）定义在
[docs/dev/ENGINEERING_WORKFLOW.md](../dev/ENGINEERING_WORKFLOW.md)。

本 contract 只定义 runtime architecture。工程流程纪律是独立文档。
