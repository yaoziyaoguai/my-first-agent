...
正式命名空间是 `agent/skill_system/`。Skill 是 filesystem-first package，默认只暴露 metadata，只有被选中时才加载 body，references/scripts/templates/tests 仍需按需加载。`agent/skills/` 是 tombstone，`agent/legacy_skills/` 是 tombstone 的 stale historical target，**该目录当前不存在**——`import agent.legacy_skills` 会 ImportError，quarantine 由目录不存在强制执行。
