...
|---|---|---:|---|---|---|---|
| Core governed runtime spine | `main.py`, `agent/core.py`, `agent/loop.py`, `tool_runtime_mediator.py`, `tool_executor.py`; golden E2E; 40 AST boundary tests; R-series Run 12; reproducible G-010/G-015 dogfood | **L5** | operator_ready. Real provider -> interactive CLI -> governed tool_use (write/edit) -> evidence/checkpoint (Run 12 + reproducible G-010/G-015). Operator surface complete: capability-status command + OPERATOR_GUIDE + troubleshooting + consistent onboarding (G-037 fixed). TOOL_INVOKE evidence-only (AST-pinned). | L6 (released) needs sustained real operator usage beyond reproducible dogfood. | Inflating to L6 before sustained usage. | Keep the spine fixed; L6 requires sustained real usage. |
...
| Interactive CLI / operator workflow | `main.py`, `agent/cli/commands.py`, `agent/cli_renderer.py`; `tests/test_cli_*`, subprocess CLI tests | **L5** | operator_ready. Operator surface complete: capability-status command (G-007), OPERATOR_GUIDE runbook + provider/evidence/governance/tool matrices (G-008/011/014/018), status/health/logs/sessions, reproducible G-010 dogfood, onboarding consistent (G-037 fixed). Run 12/14 exercised help/health/logs/demo under real provider. | L6 (released) needs sustained real operator usage. | Inflating to L6 before sustained usage. | Keep operator docs in sync; L6 requires sustained usage. |
...
| Memory | `memory_runtime.py`, `memory_store.py`, `memory_fs_store.py`, `memory_extraction.py`, `memory_consolidation*.py`, `memory_policy.py`, `memory_review.py`; ~48 test files | **L4** | Real-trigger verified (G-019 reproducible real DeepSeek memory dogfood: `MEMORY_REMEMBER_REQUEST` -> `memory_confirmation` approval -> stored -> `list_records` recall; opt-in pass). Retain/recall/forget, store backends, deterministic consolidation, evidence, confirmation, review CLI fake/local-tested. | Consolidation subsystem is architecturally FROZEN/deferred across all 6 consolidation modules (`memory_consolidation.py:15-17`); LLM consolidation default-off (`MEMORY_CONSOLIDATION_LLM_ENABLED`) — not real-verified. No CI gate (opt-in). | Privacy/retention and surprise-recall risks. | Operator review via `memory extract/index/archive`; keep consolidation frozen. |
| Skill system | `agent/skill_system/*`, `runtime_integration/skill_*.py`, `skills/demo-note-maker`; ~26 test files | **L4** | Real select/execute verified (G-022 reproducible real DeepSeek skill dogfood: SKILL_SELECT picks demo-note-maker -> `demo.write_demo_note` -> governed approval -> note written + skill evidence; opt-in pass). Boundary AST tests guard the seam. | No real external/private skill directory; only fixture/sample skills. No CI gate (opt-in). | Reading real skill dirs or letting skills bypass runtime policy would break boundaries. | Fixture/sample skills; `demo-note-maker` dogfood; keep boundary tests green. |
| MCP config / bridge | `agent/mcp_*`, `runtime_integration/mcp_*`; ~16 test files | **L4** | Real-endpoint verified (G-025: real stdio MCP flight against a safe LOCAL fixture server — StdioMCPClient initialize/list_tools/call_tool, default-run; connect/list/call/result). Config CLI/service/presenter, bridge, policy, sanitizer, audit, external-readiness tested. | Activation default-off via env-gate (`main.py:610`); full multi-server ecosystem deferred (TD-009). Real npx/external endpoint flight still opt-in (test_real_mcp_flight.py). | Real external MCP can expose secrets/remote commands; local fixture is sandboxed. | Local real flight proven; external endpoints stay opt-in + authorized. |
