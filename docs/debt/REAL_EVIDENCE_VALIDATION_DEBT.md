...
**创建日期**: 2026-05-28
**最后更新**: 2026-05-29 (006 L1 delegation path verified — root cause of SUBAGENT_DELEGATE_L0 was validation script message format, not code defect; L1 handler correctly routes to delegate_l1()→execute_l1())
...
| **Current evidence** | L1 code path complete: execute_l1() + delegate_l1() + mediate_child_tool_request() + mediate_child_memory_request()；child memory scope (none/propose) with namespaced store write；SUBAGENT_CHILD_MEMORY_REQUEST dispatcher evidence；CLI shortcuts 迁入 dispatcher path；**L1 real-model descriptor**: 新建 `demo-stat-real` with `model: inherit` + SAFE_MODELS 扩展（`"inherit"`） + 1 guard test；**dispatcher mismatch fix**: `core.py` 中 `_phase1_dispatcher` 赋值移至 CLI delegation 代码之前，使 delegation 证据落入 dogfood-injected dispatcher；**L1 evidence dispatch gaps 修复**: `SUBAGENT_CHILD_TOOL_REQUEST` dispatch in `mediate_child_tool_request()`、`SUBAGENT_CHILD_RESULT` dispatch in L1 handler、`SUBAGENT_PARENT_ADJUDICATION` dispatch in L1 handler（inject dispatcher via phase1_hook.py）；**Real provider dogfood validation (2026-05-29)**: 15 PASS / 0 FAIL / 1 CONCERN — C1 (SUBAGENT_DELEGATE_L1 success) / C3 (child result evidence) / C4 (parent adjudication) / C5 (memory scope=none expected) / C6 (L1 code path verified with real provider child loop) / C7 (25 events evidence chain: subagent.child_result + subagent.delegate_l1 + subagent.parent_adjudication)；C2 CONCERN: child 未调用 API tool_use（model 在 text 中描述工具而非 structured tool_use block），child tool mediation code path 由 31 contract tests 验证，production path 因 `core.py` 传入 `tool_mediator=None` 无法触发——这是已知限制（TOOL_MEDIATOR_GAP），不阻塞 CLOSED；37 L1 descriptor + 66 all-focused tests pass；**Validation script fix (2026-05-29)**: 根因分析确认 `scripts/real_evidence_006_subagent_real_provider.py` 的 user message 以 "请 " 前缀开头，导致 `detect_delegate_to_subagent` 的 `re.match()` 无法匹配 CLI delegation 语法。修复为 `delegate to demo-stat-real: <task>` 后 L1 handler 正确路由。**Re-run (2026-05-29)**: 5 PASS / 0 FAIL / 1 CONCERN (MODEL_BEHAVIOR_CONCERN) — M1 从 CONCERN 变为 PASS (SUBAGENT_DELEGATE_L1 dispatched status=success)，M7a/M7b 从 CONCERN 变为 PASS (child result + parent adjudication dispatched)，M2 仍为 MODEL_BEHAVIOR_CONCERN（child model 输出 XML/文本格式 tool_use 而非 API structured block——模型能力限制，非代码缺陷）。**代码路径完整**: L1 handler → delegate_l1() → execute_l1() → child provider call → parent ToolRuntimeMediator → TOOL_GATE→INVOKE→RESULT 全链路由 52 contract tests 验证。**根因**: 上一次 "M1 CONCERN" 是 validation script 消息格式问题，不是 production code bug。 |
| **Status** | **CLOSED (stronger partial-credible)** — L1 handler route 验证完毕: `_dispatch_or_fallback_delegation()` 内部构造 ToolRuntimeMediator → `set_provider()` → `delegate_l1()` → `execute_l1()` — 完整 L1 路径可用。52 contract tests PASS。**Real provider E2E (2026-05-29, 第二轮)**: `scripts/real_evidence_006_subagent_real_provider.py` — 5 PASS / 0 FAIL / 1 CONCERN (MODEL_BEHAVIOR_CONCERN)。M1 L1 delegation 正确路由 (SUBAGENT_DELEGATE_L1 success)，M7a/M7b child result + parent adjudication 已 dispatch。**根因**: 上一轮 "M1 CONCERN (L0 dispatched)" 是 validation script 消息格式问题——"请 delegate..." 前缀导致 `detect_delegate_to_subagent` regex mismatch，修复后正确路由。**剩余**: M2 child structured tool_use 需要支持 structured tool_use 的模型——当前模型输出 XML/文本格式 tool call，非代码缺陷。52 contract tests 已验证 mediation code path 正确。 |
| **Blocking current code loop** | no |
| **Blocking READY claim** | no |
| **Closed date** | 2026-05-29 |
| **Closing evidence** | contract: 52 L1 parent-mediated tests PASS；real provider E2E: `scripts/real_evidence_006_subagent_real_provider.py` (5 PASS / 1 CONCERN / MODEL_BEHAVIOR_CONCERN) — 第二轮修复后 M1 L1 delegation 正确路由；结果文件 `docs/dogfood/real-evidence-006-subagent-real-provider-results.json` |
| **Known limitation** | TOOL_MEDIATOR_GAP resolved. **Remaining**: real provider child structured tool_use E2E 受模型能力限制 (当前模型输出 XML/文本格式 tool call 而非 API structured tool_use block)；mediation code path 已由 52 contract tests 验证；SimpleNamespace turn_state + _turn_context 私有属性访问 caveat 仍在。**Previous M1 CONCERN 根因**: validation script 的 "请 delegate..." 前缀导致 `detect_delegate_to_subagent` regex mismatch，修复后 L1 handler 正确路由。 |
| **Credibility** | stronger partial-creditable — L1 delegation 路径验证完毕（5 PASS real provider），mediation code path 由 52 contract tests 验证，仅 child structured tool_use 受模型能力限制 |
