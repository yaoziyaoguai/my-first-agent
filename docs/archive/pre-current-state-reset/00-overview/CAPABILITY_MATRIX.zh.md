...
这篇文档解决什么问题：列出 First Agent 当前完成、已测试验证、gated、future 和明确不支持的能力。

不解决什么问题：不替代具体 RFC，也不授权开启 gated/future capability。

推荐读者：项目维护者、审计者、准备继续实现的 Coding Agent。

| Capability | Status | Evidence | Default | Notes |
|---|---|---|---|---|
...
| Memory explicit retain | 已完成 | focused tests | on | 必须经 confirmation |
...
| Skill System | 已完成 formal safe-local | focused tests | on for formal local contracts | 不默认安装远程 skill |
| SubAgent L0 | 已完成 | T1 synthetic 16/16 | on for local deterministic contracts | no real LLM / no shell / no external process |
| SubAgent L1 Read-Only | gated | not default | off | 需要 config + audit + approval |
...
| External process | gated/high risk | no default validation path | off by default | 需 ToolRegistry + confirmation |
...
- Evidence Tier = T1-T6，仅用于历史 SubAgent 覆盖层级描述；不作为当前行动源。
