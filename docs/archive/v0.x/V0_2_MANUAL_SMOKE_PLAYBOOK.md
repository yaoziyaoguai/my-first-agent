...
### 5.2 M6 安全现状（**§5.2 已 100% 自动化**；本节列表保留作为人工 cross-check 参考）

> 💡 **快速通道**：以下所有断言已在 `pytest -q` 中固化（见
> `docs/V0_2_RC_STATUS.md` §2.2.1 自动 smoke 覆盖快照）。
> 如果 `pytest -q` 通过，可**跳过本节**；只在仍想观察真实 CLI 交互体感
> 时手动执行。
