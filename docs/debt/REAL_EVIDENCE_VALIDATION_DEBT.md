...
**创建日期**: 2026-05-28
**最后更新**: 2026-05-29 (003/007 main-path hardened — FakeProvider deterministic evidence scripts; 001-008 CLOSED)
...
| **Status** | **CLOSED (hardened)** — disallowed-tool blocking evidence chain 通过 main runtime path (core.chat → ToolRuntimeMediator → TOOL_GATE) 验证，非 direct dispatcher.route() call；demo.echo_task_summary (confirmation="never") 被 skill_allowed_tools 约束拒绝，证明 rejection 的唯一原因是 skill 约束；5/5 PASS |
| **Blocking current code loop** | no |
| **Blocking READY claim** | no |
| **Closed date** | 2026-05-29 |
...
| **Status** | **CLOSED (hardened)** — MCP unified pipeline entry 通过 FakeProvider + main runtime path 验证：MCP 工具进入与其他工具相同的 ToolRuntimeMediator → TOOL_GATE 路径；TOOL_GATE source=ToolRuntimeMediator 确认非 direct-call；confirmation='always' 阻止 TOOL_INVOKE 是预期行为，不是路径缺陷；8 PASS / 0 FAIL / 1 CONCERN |
| **Blocking current code loop** | no |
| **Blocking READY claim** | no |
| **Closed date** | 2026-05-29 |
| **Closing evidence** | `scripts/real_evidence_007_mcp_invoke.py` — 8 PASS / 0 FAIL / 1 CONCERN (FakeProvider + real StdioMCPClient + main runtime path)；fixture `scripts/fixtures/mcp_echo_server.py`；结果文件 `docs/dogfood/real-evidence-007-mcp-invoke-results.json` |
| **Known limitation** | Model-selected MCP invocation not achieved (Guardrail 1)；confirmation='always' blocks TOOL_INVOKE execution；pipeline ENTRY (TOOL_GATE) 已验证，full pipeline (TOOL_GATE→TOOL_INVOKE→TOOL_RESULT with execution) 需修改 confirmation policy 或模型自主确认 |
