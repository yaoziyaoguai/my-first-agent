...
**最后更新**: 2026-05-31 (Loop 4 redesign: 移除中文 bigram/partial-match 跨语言 hack，retriever 改为 manifest 显式声明语言匹配。D03 重分类为 manifest language coverage decision。22/22 retriever tests PASS。**002 建议状态: credible-with-caveats**。剩余 D04/D05/D06 均为 MODEL_BEHAVIOR 或下游依赖。)
**状态**: 阶段性收口。Score 4.1/5 conservative baseline with caveats。Evidence collected: 8/8。Credible: 6/8 (001/004/005/006/007/008)。Credible-with-caveats: 1/8 (002)。Partial-credible: 1/8 (003, see caveats below)。002: **credible-with-caveats / Plan 3 pipeline + manifest-based language matching** — Plan 3 (Phase 2-7) complete: SkillCandidateRetriever + selection section (wired into production runtime, D01/D02 PASS) + ActiveSkillLifecycle + allowed_tools enforcement + real provider dogfood (9 PASS / 3 CONCERN / 0 FAIL) + B7 extension points。**Language matching**: 不做隐式跨语言匹配。Skill 用什么语言声明 metadata 就按什么语言匹配。英文 skill 不自动匹配中文 prompt（expected-by-design）。多语言支持由 manifest 显式声明中文 aliases/triggers。后续 slash command 提供显式 skill 调用路径。**Caveats**: (1) prompt-steered; (2) 单 skill 场景; (3) D04 CONCERN: lifecycle activation evidence; (4) D05 CONCERN: TOOL_GATE payload alignment; (5) D06 CONCERN: MODEL_BEHAVIOR (negative_triggers 在 retriever 层生效)。**Loop 4 (2026-05-31)**: 移除 _chinese_bigram_overlap/_chinese_partial_match/_contains_chinese，22/22 tests PASS。003: **partial-credible / code-path credible with blocking demonstrated** — real SKILL_SELECT activates demo-note-maker → adversarial prompt 诱导 SKILL_SELECT("blog-writing") → TOOL_GATE rejects via skill_allowed_tools→rejected → no TOOL_INVOKE → FORCE_STOP (5/5 PASS)。**Caveats**: (1) adversarial prompt 极度 prompt-steered: 直接要求调用 SKILL_SELECT 并设置 blog-writing; (2) 被阻断工具是 SKILL_SELECT 自身，非更自然的 disallowed 业务工具; (3) policy_path/rejection_reason 在 handler payload 中不在 RuntimeActionEvent.evidence，当前用 decision=rejected + skill_allowed_tools present 作为语义等价代理; (4) 单 skill/单工具/单 adversarial prompt 场景。006: **credible — 完整 evidence chain 闭合 (12/12 PASS)**。007: **credible — runtime-mediated execution chain 已验证** (10/10 PASS)。008: **credible — evidence chain fully closed, 最后一个 caveat 已关闭 (2026-05-31 v3)**: `core.chat()` → `_run_planning_phase()` ActionPlan schema enforcement working → real model outputs stable ActionPlan JSON → scheduler → 14/14 PASS。**不再有 MODEL_BEHAVIOR_CONCERN**。6 处 `Plan.model_validate` ActionPlan guard 全部就位。
...
| REAL-EVIDENCE-002 | Skill selection | **credible-with-caveats / Plan 3 pipeline + manifest-based language matching** | Plan 3 Phase 3-7 complete: turn-start structured selection + ActiveSkillLifecycle + allowed_tools enforcement + real provider dogfood (9 PASS / 3 CONCERN) + B7 extension points。74 focused tests PASS。**Loop 4 (2026-05-31)**: 移除中文 bigram/partial-match 跨语言 hack，retriever 只匹配 manifest 显式声明的 metadata。Do3 重分类为 manifest language coverage decision（非 runtime gap）。22/22 retriever tests (含 English-only no match for Chinese / Chinese manifest matches Chinese / Math no_skill)。D01/D02 PASS。**Caveats**: (1) prompt-steered; (2) 单 skill 场景; (3) D04/D05/D06 CONCERN — MODEL_BEHAVIOR 或下游依赖。Not product-ready。 |
